...
def main():
    ...
    with open("testing-data.csv","r") as test:
        ...
        for row in csv_reader:
            ...
            pkt = pkt /IP(dst=addr) / TCP(dport=1234, sport=random.randint(49152,65535)) / Klass(hash=0, X10=int(row[10]),X11=int(row[11]),X14=int(row[14]),X17=int(row[17]),X27=int(row[27]), start=int(time.time())
